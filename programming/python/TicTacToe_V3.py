#Ende des Projekts mit offenen Baustellen:
#Draw Lösung fehlt
#Runden Counter Stoppt nicht - Spiel geht undendlich - Rundenabfrage ist quatsch

import random
game = True
gamewon = False
gamerwantstoplay = True
sfeld = ["1","2","3","4","5","6","7","8","9"]
failmessage = "Bitte gibt eine Zahl von 1-9 ein, die noch nicht Belegt ist!"
...
def spielzug():
    global p1zug,p2zug,p1s,p2s,p1wrate,p2wrate #Anfang der Spielerzuodrnung
    if p1 in p1info:
        p1s = p1info[1]
        p2s = p2info[1]
    else:
    ...
    while not gamewon == True:
        ...
        while not p1input == True:
            ...
            try:
                ...
                if p1zug >= 1 and p1zug <=9:
                    p1zugzahl = int(p1zug) #Spielereingabe wird verarbeitet
                    if not sfeld[p1zugzahl-1] == p1s and not sfeld[p1zugzahl-1] == p2s:
                        p1input = True
                    else:
                        print(failmessage)
                else:
        ...
        if not gamewon == True: #Spieler 2 Spielzug
            ...
            while not p2input == True:
                ...
                try:
                    ...
                    if p2zug >= 1 and p2zug <=9:
                        p2zugzahl = int(p2zug) #Spielereingabe wird verarbeitet
                        if not sfeld[p2zugzahl-1] == p1s and not sfeld[p2zugzahl-1] == p2s:
                            p2input = True
                        else:
                            print(failmessage)
                    else:
...
while game == True:
    spielerinfo()
    while not gamerwantstoplay == False:
        roundstowin()
        starterchoice()
        spielstand()
        spielzug()
        print("Total Score",p1,p1wrate,"/",p2wrate,p2)
        print("Wollen sie noch eine Runde Spielen? True/False")
        gamerwantstoplay == input("--> ")
        if gamerwantstoplay == False:
            game = False